...
import { getUserInfoFromProfiles } from "@/lib/functions";
...
const ProfilePage = ({ params }: { params: { username: string } }) => {
  ...
  const [userBgIsLoaded, setUserBgIsLoaded] = useState(false);

  useEffect(() => {
    const fetchUserData = async () => {
      try {
        setIsLoading(true);
        const sanitizedUsername = username.startsWith("%40")
          ? username.slice(3)
          : username;
        ...
        setEditAvatar(userProfileData?.pfp_link || "");
        setEditBackground(userProfileData?.bg_link || "");
        setRole(
          userProfileData?.role.charAt(0).toUpperCase() +
            userProfileData?.role.slice(1)
        );
      ...
      try {
        ...
        if (sessionData?.session) {
          ...
          if (userProfileError || !userProfile || userProfile.length !== 1) {
            console.error(
              "Error fetching user profile data or multiple rows returned:",
          ...
          if (newRelationError) {
            console.error(
              "Error creating new relation:",
              newRelationError.message
            );
            setEditRelation("Follow");
            toast({
              variant: "destructive",
  ...
  async function handleEditProfile() {
      ...
      if (loggedInUserId === userData?.id) {
        if (!editName) {
          ...
          toast({
            variant: "destructive",
            title: "Error",
            description: "Your role cannot be empty.",
          });
          return;
        } else {
    ...
    getUserInfoFromProfiles("name", true);
  }
  ...
  return (
    <AnimatePresence>
      ...
      >
        <div className="relative w-full max-h-40 min-h-40">
          {!userBgIsLoaded && (
            ...
            <Image
              src={
          userData?.bg_link ||
          "https://shop.wallpaperpaintandmorestl.com/cdn/shop/products/454F57_1024x.png?v=1607636420"
              }
              alt="User Profile Banner"
              className="w-full h-full object-cover"
              quality={100}
              fill={true}
              onLoadingComplete={() => setUserBgIsLoaded(true)}
            />
        </div>
        ...
        <div className="relative flex flex-col md:flex-row items-center md:items-start text-black dark:text-white md:ml-16">
          ...
          <div className="flex flex-col items-center md:items-start gap-3 md:ml-5 mt-3">
            ...
            <div className="flex items-center gap-3">
              <Badge variant="default">@{userData?.username || "username"}</Badge>
                ...
                <DialogContent>
                  ...
                  <div className="p-3">
                    ...
                    <DialogFooter className="gap-3">
                      <DialogClose asChild>
                        <Button variant="destructive" id="closeProfileEdit">
                          Cancel
                        </Button>
                      </DialogClose>
        ...
        <div className="mt-6 md:mx-16 p-3">
          ...
          <p className="text-black dark:text-white mt-2">
            {userData?.bio || "This user has not set a bio"}
          </p>
        </div>
        ...
        <div className="mt-6 md:mx-16 p-3">
          ...
          <p className="text-neutral-600 dark:text-neutral-400 mt-2">
            This user has no publications or public projects
          </p>
        </div>
      </motion.div>
    </AnimatePresence>
